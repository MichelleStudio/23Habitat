...
function preload() {
  ...
  countImg['BV'] = loadImage("peripheral/BV.png");
  countImg['CA'] = loadImage("peripheral/CA.png");
  countImg['CS'] = loadImage("peripheral/CS.png");
  
  for (let i = 0; i < ftype.length; i++) {
    for (let j = 0; j < fset.length; j++) {
      for (let k = 0; k < fcolor.length; k++) {
          foliage[ftype[i]].push(loadImage('foliage/'+ftype[i]+fset[j]+fcolor[k]+'.png'));
      }
    }
  }
